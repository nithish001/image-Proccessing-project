function affineinvpts = harrisaffine(img,scaleinvpts)
  ...
  for i=1:1
    ...
    while epsilon_c > 0.05 || loop == 0
      ...
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      %% Step 2
      xw_kminus1 = round(transformBackward(xk,U_k)) %xw = [U^(-1)]x -> xw is the point in the transformd Image
    ...
    imshow(img) 
    hold on
    xCenter = affineinvpts(:,2);
    yCenter = affineinvpts(:,1);
    xRadius = 2.5;
    yRadius = 8;
    theta = 0 : 0.01 : 2*pi;
    ...
    ylim([0 20]);
    grid on;
%     plot(affineinvpts(:,2),affineinvpts(:,1),'gs')
